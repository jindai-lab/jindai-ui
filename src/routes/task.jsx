import { CheckCircleOutlined, ClockCircleOutlined, CloseCircleOutlined, DeleteOutlined, EditOutlined, ExclamationCircleOutlined, PlayCircleOutlined, SyncOutlined } from '@ant-design/icons';
import { Alert, Button, Card, Col, Collapse, Descriptions, Divider, Empty, message, Modal, Row, Space, Spin, Statistic, Table, Tag, Typography, Popconfirm } from "antd";
...
const TaskDboList = () => {
  ...
  const runTask = async (task_id) => {
    const jobId = await api.callAPI('worker/custom', { task_id }, { method: 'POST' })
    console.log(jobId)
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      render: (record) => {
        return (<>
          <Button size="small" icon={<EditOutlined />} onClick={() => navigate(`/tasks/${record.id}`)}>编辑</Button>
          <Button size="small" icon={<PlayCircleOutlined />} onClick={() => runTask(record.id)}>运行</Button>
        </>)
      }
    }
  ];
...
function JobDetail({ job, jobId, visible, onCancel }) {
  ...
  const renderErrorChain = (errorObj, level = 0) => {
    if (!errorObj) return null;
    const indentStyle = { marginLeft: `${level * 16}px`, marginTop: '8px' };

    return (
      <div style={indentStyle}>
        <Text strong>错误类型：</Text>
        <Text type="danger">{errorObj.exc_type}</Text>
        <br />
        <Text strong>错误模块：</Text>
        <Text>{errorObj.exc_module}</Text>
        <br />
        <Text strong>错误信息：</Text>
        <Paragraph
          style={{ margin: '4px 0' }}
          ellipsis={{ rows: 3, expandable: true, symbol: '查看全部' }}
        >
          {errorObj.exc_message?.join('\n') || '无'}
        </Paragraph>
        ...
        {errorObj.exc_cause && (
          <>
            ...
            {renderErrorChain(errorObj.exc_cause, level + 1)}
          </>
        )}
      </div>
    );
      ...
      {!job ? (
        ...
        <div style={{ padding: '10px 0' }}>
          ...
          >
            ...
            <Descriptions.Item label="执行时间(秒)">
              {job.execution_time} s
            </Descriptions.Item>
            <Descriptions.Item label="日志内容" span={2}>
              {job.log ? (
                <Paragraph
                  ellipsis={{ rows: 2, expandable: true, symbol: '查看全部日志' }}
                  style={{ margin: 0 }}
                >
                  {job.log}
                </Paragraph>
...
function JobsList() {
  ...
  const jobColumns = [
    ...
    },
    {
      ...
      dataIndex: 'is_err',
      key: 'is_err',
      render: (is_err) => (is_err ? (<Tag icon={<ExclamationCircleOutlined />} color="error">
        执行失败
      </Tag>
      ) : (
        <Tag icon={<CheckCircleOutlined />} color="success">
    ...
    >
      <Panel header="结果详情" key="detailed">
        <Table
          ...
          bordered
        />
        <JobDetail jobId={currentJobId} job={jobDetail} visible={modalVisible} onCancel={() => setModalVisible(false)} />
      </Panel>
    </Collapse>)
}
