...
const PdfViewer = ({ splat }) => {
    ...
    useEffect(() => {
        fetch(`/api/files/${encodeURIComponent(splat)}?metadata=true`)
    ...
    return (
        <>
            ...
            <div className="pdf-viewer">
                ...
                <SinglePagePDFViewer fileUrl={`/api/files/${encodeURIComponent(splat)}?page=${pdfPage - 1}`} width={document.getElementsByClassName('.single-page-pdf-viewer')[0]?.clientWidth} />
        ...
        case 'htm':
            return (
                <div>
                    ...
                    <iframe
                        title={splat}
                        src={`/api/files/${encodeURIComponent(splat)}`}
