<template>
  <div class="ml-10">
    ...
    <draggable handle=".drag-handle" :list="value">
      <div v-for="(stage, index) in value" :key="index" class="stage-opers">
        <StageDetail
          :stages="stages"
          :map_id="map_id + '.' + index"
          v-model="value[index]"
        ...
        <div class="opers">
          ...
          <v-btn icon @click="move_stage(index, -1)" v-show="index > 0">
            <v-icon>mdi-arrow-up</v-icon>
          </v-btn>
          <v-btn @click="move_stage(index, 1)" v-show="index < value.length - 1" icon>
...
export default {
  ...
  methods: {
    ...
    append_stage(index) {
      if (typeof index === "undefined") this.value.push(["Passthrough", {}]);
      else this.value.splice(index, 0, ["Passthrough", {}]);
    },
