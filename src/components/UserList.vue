    ...
    <v-dialog v-model="show_modal">
      <v-card>
        <v-card-title v-if="show_modify">
          修改用户角色
          <v-spacer></v-spacer>
          <v-btn icon @click="show_modal = false"><v-icon>mdi-close</v-icon></v-btn>
        </v-card-title>
        <v-card-title v-else>添加用户</v-card-title>
      <v-card-text>
        <v-text-field label="用户名" v-model="new_user.username" :disabled="show_modify"></v-text-field>
        <v-text-field label="密码" type="password" v-model="new_user.password" :disabled="show_modify"></v-text-field>
        ...
        <v-checkbox label="用户及其他管理" value="admin" v-model="new_user.roles"></v-checkbox>
        <v-btn @click="modify_user_role" color="primary" v-if="show_modify">修改</v-btn>
        <v-btn @click="add_user" color="primary" v-else>确定</v-btn>
        <v-btn @click="show_modal = false" class="ml-5">取消</v-btn>
      </v-card-text>
      </v-card>
    </v-dialog>
  </v-card>
