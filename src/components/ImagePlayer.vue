<template>
  <div class="viewer-parent" ref="viewerBox"
...
export default {
  ...
  methods: {
    ...
    apply_transform(transf) {
      ...
      var width = this.$img.naturalWidth * scale, height = this.$img.naturalHeight * scale;
      if (degree) {var t= height;height=width;width=t;}
      this.$dom.style.width = width + 'px',
    this.$dom.style.height = height + 'px',
        this.$dom.style['margin-left']= x+'px'
         this.$dom.style['margin-top']=y+'px'
      this.$img.style['transform-origin'] = 'top left'
      var trs = ([(degree ? `translate(${width}px, 0) rotate(90deg)` : ""), `scale(${scale})`].join(' '))
    ...
    viewer_mousemove(e) {
      if (!e.ctrlKey || !this.mouse.enabled)return
