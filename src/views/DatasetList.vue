  ...
  <v-card flat>
    ...
    <v-card-text>
      <v-data-table :items="datasets" :search="search" :headers="[
        ...
        <template v-slot:item.tag="{ item }"><v-combobox v-model="item.tags" flat multiple chips dense :items="all_tags"
            deletable-chips @change="update_tags(item)"></v-combobox>
        </template>
  ...
  methods: {
    ...
    save() {
      ...
      this.datasets.forEach((x, i) =>
        batch[x._id] = Object.assign(x, { order_weight: i, sources: null })
      )
      this.business.datasets({ update: batch }).then(() => this.$notify(this.$t("saved")));
    },
    append_dataset() {
      if (!this.input_coll.name) return;
      this.business.datasets({ creation: this.input_coll }).then(() => {
        this.load_datasets();
        this.input_coll = {};
      });
    },
    ...
    update_tags(ds) {
      this.business.datasets({ update: { _id: ds._id, tags: ds.tags } })
