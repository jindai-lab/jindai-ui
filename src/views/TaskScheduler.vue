  ...
  <v-card flat>
    <v-card-title>计划任务
      ...
      <v-btn color="primary" @click="show_creation = true;">
        <v-icon>mdi-plus</v-icon> 建立计划任务
      </v-btn>
    </v-card-title>
      <v-card-text>
        <v-row v-for="task in scheduled_jobs" :key="task.key" class="ma-3">
            {{ task.key }} : {{ task.name }} ({{ task.repr }})
            <v-btn @click="delete_schedule(task.key)"><v-icon>mdi-delete</v-icon> 取消</v-btn>
        </v-row>
      </v-card-text>
      <v-dialog v-model="show_creation">
        <v-card>
					<v-card-title>添加计划任务</v-card-title>
          <v-card-text>
            <ParamInput v-model="scheduler.task" :arg="{type: 'TASK', name: '要定时执行的任务'}" />
            <ParamInput v-model="scheduler.cron" :arg="{type: 'str', name: '定时说明，如 day at 06:00'}" />
          </v-card-text>
					<v-card-actions><v-btn @click="create_schedule" color="primary">
              <v-icon>mdi-plus</v-icon>
              添加</v-btn>
							<v-btn @click="show_creation = false;">取消</v-btn>
							</v-card-actions>
				</v-card>
      </v-dialog>
  </v-card>
...
export default {
  ...
  methods: {
    update_scheduler() {
        api.call('scheduler').then(data => 
          this.scheduled_jobs = data.result
        )
    },
    create_schedule() {
